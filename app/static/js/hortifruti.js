...
window.BrigadaHortifruti = {
        ...
        <div class="modal">
          ...
          <div class="modal-body">
            <form id="product-form-hortifruti">
              ...
              <div class="form-row" id="annotation-info-banner-hortifruti" style="display: none; margin-top: 1rem; width: 100%;">
                <div style="background: rgba(99, 102, 241, 0.15); border: 1px solid rgba(99, 102, 241, 0.3); border-radius: 6px; padding: 8px 12px; width: 100%; display: flex; justify-content: space-between; font-size: 0.85rem; color: #f8fafc;">
                  <div><strong>Quantidade Anterior:</strong> <span id="info-original-quantity-hortifruti">0</span></div>
                  <div><strong>Data da Alteração:</strong> <span id="info-change-date-hortifruti">--/--/----</span></div>
                </div>
              </div>
              <div class="form-row" id="group-annotation-hortifruti" style="display: none; margin-top: 1rem;">
                <div class="form-group" style="width: 100%;">
                  ...
                  <input type="hidden" id="field-annotation-hortifruti" value="">
                    <input type="text" id="field-annotation-text-hortifruti" class="form-input" style="flex: 2; display: none;" placeholder="Detalhes (obrigatório)" maxlength="100">
                  </div>
                </div>
              </div>
            </form>
  ...
  bindEvents(container) {
    ...
    const qtyInput = container.querySelector('#field-quantity-hortifruti');
    const annotationGroup = container.querySelector('#group-annotation-hortifruti');
    const annotationInput = container.querySelector('#field-annotation-hortifruti');
    const annotationTextInput = container.querySelector('#field-annotation-text-hortifruti');
    
    qtyInput?.addEventListener('input', () => {
      if (this.editingId) {
        const newVal = parseFloat(qtyInput.value) || 0;
        const infoBanner = container.querySelector('#annotation-info-banner-hortifruti');
        if (newVal < this.originalQuantity) {
          annotationGroup.style.display = 'block';
          annotationInput.required = true;
          if (infoBanner) {
            container.querySelector('#info-original-quantity-hortifruti').textContent = this.originalQuantity + ' ' + (container.querySelector('#field-unit-hortifruti').value || '');
            const today = new Date();
            const dd = String(today.getDate()).padStart(2, '0');
            const mm = String(today.getMonth() + 1).padStart(2, '0');
            const yyyy = today.getFullYear();
            container.querySelector('#info-change-date-hortifruti').textContent = dd + '/' + mm + '/' + yyyy;
  ...
  openModal(container, id = null) {
    ...
    // Reset annotation
    container.querySelector('#group-annotation-hortifruti').style.display = 'none';
    container.querySelector('#field-annotation-hortifruti').required = false;
    const infoBanner = container.querySelector('#annotation-info-banner-hortifruti');
    if (infoBanner) infoBanner.style.display = 'none';
    ...
    container.querySelector('#field-annotation-hortifruti').value = '';
    const annText = container.querySelector('#field-annotation-text-hortifruti');
    if (annText) {
    ...
    if (id) {
      ...
      form.querySelectorAll('.form-input, select, textarea').forEach(el => {
        if (el.id !== 'field-quantity-hortifruti' && el.id !== 'field-annotation-hortifruti' && el.id !== 'field-annotation-text-hortifruti') {
          el.disabled = true;
        } else {
          el.disabled = false;
        }
  ...
  async saveProduct(container) {
    ...
    const quantity = container.querySelector('#field-quantity-hortifruti').value !== '' ? parseFloat(container.querySelector('#field-quantity-hortifruti').value) : 0;
    const selectVal = container.querySelector('#field-annotation-hortifruti').value;
    const textVal = container.querySelector('#field-annotation-text-hortifruti')?.value.trim() || '';
